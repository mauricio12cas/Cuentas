  ...
  state.weeks.forEach(w => {
    w.entries.forEach(e => {
      if (e.cafeteria && e.cafeteria.trim()) {
        uniqueNames.add(e.cafeteria.trim());
      }
    });
  });
...
const cafeteriaInput = document.getElementById('modal-cafeteria-input');
...
function openRecordModal(entryId = null) {
  ...
  if (entryId) {
    ...
    if (entry) {
      cafeteriaInput.value = entry.cafeteria;
      facturaInput.value = entry.factura || '';
      totalInput.value = entry.total;
      pagoValInput.value = entry.pago;
      
      // Render payment day pills selecting the current entry value
      renderPaymentDayPills(entry.pago);
      
      // Highlight matching cafeteria pill if any
      const matchingPill = Array.from(cafeteriaPillsContainer.querySelectorAll('.pill-btn'))
        .find(pill => pill.textContent.trim().toLowerCase() === entry.cafeteria.toLowerCase());
      if (matchingPill) {
...
function renderCafeteriaPills() {
  cafeteriaPillsContainer.innerHTML = '';
  
  if (state.cafeterias.length === 0) return;
  ...
  state.cafeterias.forEach(cafeteria => {
    ...
    btn.addEventListener('click', () => {
      ...
      cafeteriaInput.value = cafeteria;
      totalInput.focus();
    });
...
// Clear selected pill if user types custom text
cafeteriaInput.addEventListener('input', () => {
  const text = cafeteriaInput.value.trim().toLowerCase();
  cafeteriaPillsContainer.querySelectorAll('.pill-btn').forEach(btn => {
    if (btn.textContent.trim().toLowerCase() === text) {
      btn.classList.add('active');
    } else {
      btn.classList.remove('active');
    }
